...
- Follow `.agents/skills/graphite/SKILL.md` and the Graphite flow in
  `.cursor/rules/pr-workflow.mdc` (`gt`, `scripts/dev/submit-stack`).
...
2. Rewrite `.cursor/rules/pr-ship-and-review.mdc` submit block to the marker-aware
   template under `scripts/lib/repo-practices-cursor/pr-ship-and-review.mdc`.
3. Delete root `GRAPHITE.md` when switching to `gh-stack` (skills here are SSOT).
4. Keep `.cursor/rules/stacking-tool.mdc` aligned with the consumer stacking-tool template.
...
`.cursor/rules/pr-ship-and-review.mdc` / `.agents/skills/ship-and-review/SKILL.md`.
