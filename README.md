...
locally: a missing entry FAILs `--new-repo`, a stale one FAILs `--strict-onboarding`;
CI runs (no ledger on the runner) skip the check. The daily timer **does** maintain
the ledger.
...
| Pre-PR checks cursor rule | yes* | — | `.cursor/rules/pre-pr-checks.mdc`: format before check + no truncated output; consumer repos resolve `scripts/dev/` via the repository-helpers clone (#579) (*missing FAILS `--new-repo` only; SUGGEST under org `--strict-onboarding` during roll-out; invalid FAILS `--new-repo` / `--strict-onboarding`) |
...
| Secret-audit intake ledger | yes | — | Host-local `~/scratch/repository-helpers/secret-audit-intake.json` entry (`status=clean`, fresh) after a TruffleHog deep scan; missing FAILs `--new-repo`, stale FAILs `--strict-onboarding`; not checked in CI ([docs/secret-audit-trufflehog.md](docs/secret-audit-trufflehog.md)) |
