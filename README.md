...
| Actions hardening | yes* | — | `default_workflow_permissions: read`, `can_approve_pull_request_reviews: false`, and `sha_pinning_required: true` (`--apply-fix` sets the first two always; enables `sha_pinning_required` only when every third-party workflow `uses:` is already SHA-pinned — repository-helpers#640); `allowed_actions` is SUGGEST-only always (*those three FAIL under `--new-repo` and `--strict-onboarding`; SUGGEST in routine `--all` / `--suggest` — org-wide rollout complete for workflow perms, repository-helpers#588) |
