...
| Pre-PR checks agents rule | yes* | — | `.agents/rules/pre-pr-checks.md` (+ Cursor shim): format before check + no truncated output; consumer repos resolve `scripts/dev/` via the repository-helpers clone (#579) (*same #637 dual-path severity as agent-review; bare-path FAILS `--new-repo` / `--strict-onboarding` (#575/#579); routine `--all` / `--suggest` emits SUGGEST and succeeds) |
| Git commit identity agents rule | yes* | — | `.agents/rules/git-commit-identity.md` (+ Cursor shim) forbids agent/machine co-authors; agents must verify commit signing and `~/.cursor/cli-config.json` attribution (*#637 dual-path severity) |
...
| Stacking-tool marker + rule | yes* | — | `.github/stacking-tool` (`graphite`\|`gh-stack`) and `.agents/rules/stacking-tool.md` + thin Cursor shim (*#637 dual-path) |
...
| Actions hardening | yes* | — | `default_workflow_permissions: read` and `can_approve_pull_request_reviews: false` (`--apply-fix` sets both); `allowed_actions` is SUGGEST-only always; `sha_pinning_required` is SUGGEST-only for severity but `--apply-fix` enables it when every third-party workflow `uses:` is already SHA-pinned (repository-helpers#640) (*`default_workflow_permissions` / `can_approve_pull_request_reviews` FAIL under `--new-repo` and `--strict-onboarding`; SUGGEST in routine `--all` / `--suggest` — org-wide rollout complete, repository-helpers#588) |
