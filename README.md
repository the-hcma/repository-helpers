...
| Pre-PR checks cursor rule | yes* | — | `.cursor/rules/pre-pr-checks.mdc`: format before check + no truncated output; consumer repos resolve `scripts/dev/` via the repository-helpers clone (#579) (*missing or invalid FAILS `--new-repo` / `--strict-onboarding` (#575); the consumer helper-path re-sync is SUGGEST-only) |
