...
`setup-service` and `scripts/on-deploy` are **worktree-aware**: service templates in
`share/systemd-unit-templates/`, `@@REPO_DIR@@` substitution in the generated unit
file, and the `on-deploy` hook itself are resolved from whichever worktree the
