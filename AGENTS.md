...
- **pnpm grandfathering (cutover only).** When enabling `minimumReleaseAge` on a repo with an existing lockfile, run `scripts/grandfather-pnpm-release-age --wildcard` once to set `minimumReleaseAgeExclude: ["*"]` so the current lockfile keeps passing CI under pnpm 11.1.3+. **Forward** bumps are gated by Dependabot cooldown and dep-updater—not by ongoing grandfather maintenance. Optional `scripts/prune-pnpm-release-age-grandfather` exists only for repos that later choose to tighten pnpm install enforcement; the default fleet setup does not use it.
