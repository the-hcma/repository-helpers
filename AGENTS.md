...
- **pnpm grandfathering.** When enabling `minimumReleaseAge` on a repo with an existing lockfile, run `scripts/grandfather-pnpm-release-age` to copy current `name@version` pins into `minimumReleaseAgeExclude` so CI keeps working; only new resolutions must age out.
