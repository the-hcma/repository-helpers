...
- **CVE-marked bumps bypass release age in dep-updater only.** When `npm audit` or `pip-audit` reports a package with CVE IDs and an available fix, dep-updater skips the 9-day gate for that package (`npm_pkg_has_cve_fix_in_audit` / `py_pkg_has_cve_fix_in_audit`). Dependabot is unchanged: security update PRs already ignore `cooldown`; version-update PRs still wait 10 days.
