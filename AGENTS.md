...
2. Load the library via `on-deploy-deps-bootstrap` (see `scripts/lib/on-deploy-deps-load.snippet` — path-resolve and `source` the bootstrap file, then `on_deploy_deps_load_library "$repo_dir"`).
