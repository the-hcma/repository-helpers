...
| Pre-PR checks cursor rule | yes* | — | `.cursor/rules/pre-pr-checks.mdc`: format before check + no truncated output; **consumer** repos must resolve `scripts/dev/` through the repository-helpers clone (`${REPOSITORY_HELPERS_DIR:-$HOME/work/ai/repository-helpers}`), not a bare repo-relative path (#579) (*missing or invalid FAILS `--new-repo` / `--strict-onboarding` (#575); the consumer helper-path re-sync is SUGGEST-only) |
...
| Agent bootstrap (agent-agnostic) | yes* | — | `AGENTS.md` at root + a session-startup line telling the agent to load `.cursor/rules/*.mdc`; `CLAUDE.md` (an `@AGENTS.md` import, or a symlink to `AGENTS.md`) and `.github/copilot-instructions.md` shims so Claude Code / Copilot reach the same guidance (templates in `scripts/lib/repo-practices-agents/`) (*missing `AGENTS.md` / `CLAUDE.md` / copilot shim FAILS `--new-repo` / `--strict-onboarding` (#575); the startup-line wording stays `--new-repo`-only pending a re-sync) |
