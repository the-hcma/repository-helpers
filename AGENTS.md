...
- **npm minimum release age (10 days).** dep-updater does not propose bumps to registry versions published less than **10 days** ago (`scripts/lib/release-age-defaults`). When the target project sets a higher `minimumReleaseAge` in `pnpm-workspace.yaml`, the larger value is used. Skips align with pnpm `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION` and Dependabot `cooldown` on managed repos.
