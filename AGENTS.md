...
- **npm minimum release age (10 days).** dep-updater does not propose bumps to registry versions published less than **10 days** ago (`dep_updater_minimum_release_age_minutes=14400`). When the target project sets a higher `minimumReleaseAge` in `pnpm-workspace.yaml`, the larger value is used. Skips align with pnpm `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION` and Dependabot `cooldown` on managed repos.
- **CVE-marked npm bumps bypass release age in dep-updater only.** When `npm audit` reports a package with CVE IDs and an available fix, dep-updater skips the 10-day gate for that package (`npm_pkg_has_cve_fix_in_audit`). Dependabot is unchanged: security update PRs already ignore `cooldown`; version-update PRs still wait 10 days.
- **pnpm grandfathering.** When enabling `minimumReleaseAge` on a repo with an existing lockfile, run `scripts/grandfather-pnpm-release-age` (default `--wildcard` writes `minimumReleaseAgeExclude: ["*"]` for pnpm 11.1.3+ CI). The registry file `pnpm-release-age-grandfather.tsv` records policy; use `--tracked` to seed per `pkg@version` excludes with publish epochs.
- **Aging out grandfathered excludes.** Run `scripts/prune-pnpm-release-age-grandfather` periodically (or after lockfile refreshes). It drops entries older than `minimumReleaseAge`, syncs `pnpm-workspace.yaml`, and removes the exclude block when none remain. From wildcard mode, run `prune --expand-wildcard` first to switch to tracked excludes, then prune again as versions mature. Verify with `pnpm install --frozen-lockfile` before merging shrink PRs.
