...
| GitHub-native security settings | yes* | — | Dependabot alerts, Dependabot security updates, secret scanning + push protection (free on public repos, GHAS-gated on private — advisory only there), private vulnerability reporting; `--apply-fix` toggles each via the GitHub API (*missing/off FAILS `--new-repo` only; SUGGESTs in routine `--all` / `--suggest` and under `--strict-onboarding` until org-wide rollout completes — repository-helpers#588) |
| Actions hardening | yes* | — | `default_workflow_permissions: read` and `can_approve_pull_request_reviews: false` (`--apply-fix` sets both); `allowed_actions` and `sha_pinning_required` are SUGGEST-only (no safe default to auto-apply) (*missing/off FAILS `--new-repo` only; SUGGESTs in routine `--all` / `--suggest` and under `--strict-onboarding` until org-wide rollout completes — repository-helpers#588) |
| Actions workflow `uses:` pinning | yes* | — | Third-party `uses:` refs pinned to a full commit SHA; a release/publish workflow (holds OIDC / write tokens) with a non-SHA pin is promoted past a generic suggestion (*generic non-SHA pins are SUGGEST always; release/publish workflow pins FAIL only under `--new-repo` (SUGGEST even under `--strict-onboarding` until org-wide rollout completes, so the nightly enforcer stays green) — repository-helpers#588) |
| Deployment environment protection | yes* | — | Any environment a workflow deploys to must have a protection rule (required reviewer) or a `deployment_branch_policy` (*missing FAILS `--new-repo` only; SUGGESTs in routine `--all` / `--suggest` and under `--strict-onboarding` until org-wide rollout completes — repository-helpers#588) |
